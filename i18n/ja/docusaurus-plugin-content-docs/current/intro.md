...
地図ベースのモバイルアプリを作るのは、一見すると「地図を表示して、ピンを置いて、ルートを描くだけ」のように思えます。しかし実際には、開発者はすぐに大きな複雑さに直面します。各地図 SDK はそれぞれ異なる API と挙動、プラットフォーム固有の違いを持っており、アプリを Android と iOS の両方で動かそうとしたり、マッププロバイダを切り替えようとすると、同じロジックを書き直すことになりがちです。

MapConductor は、この摩擦を減らすために生まれました。開発者の視点から見ると、よく使う地図機能のための一貫した API を 1 つ覚えれば済みます。その裏側では、MapConductor が各ベンダーの Android 向け SDK とのやり取りを肩代わりします。最初は 1 つのプロバイダから始めて、後から別のプロバイダへ切り替えることも、コードを捨てずに行えます。

このプロジェクトは、一般的な位置情報アプリケーション向けに設計されています。デリバリーアプリ、現場作業向けツール、ニッチな可視化アプリなど、初心者の Android / iOS 開発者でも地図機能に取り組みやすくすることを目標としています。
